    ...
    ui(new Ui::SetClockDomains)
{
    ...
    ui->clockDomainSetGUI->setVisible(0);
...
void SetClockDomains::submitRouterMatrixConfigSlot(const QStringList &routerMatrixData) {
    qDebug() << "--------------------- SetClockDomains --------------------";
    qDebug() << routerMatrixData;

    rows = routerMatrixData[0].toInt();
    cols = routerMatrixData[1].toInt();
...
void SetClockDomains::on_generate_clicked()
{
    ...
    QVBoxLayout *Vlayout = new QVBoxLayout();
    Vlayout->setObjectName("WidgetInnerLayout");

    if (!firstTime) {
//        delete controlsRestrictorWidget->findChild(QVBoxLayout );
//        controlsRestrictorWidget->clearMask();
//        delete Vlayout;
//        QVBoxLayout *Vlayout = new QVBoxLayout();
//        qDeleteAll(Vlayout->findChildren<QHBoxLayout *>());
//        qDeleteAll(Vlayout->objectName());
//        delete QVBoxLayout("WidgetInnerLayout");

//        QList<QVBoxLayout *> layoutsOld = controlsRestrictorWidget->findChildren<QVBoxLayout *>();
//        foreach(QVBoxLayout * vlayoutOld, layoutsOld)
//        {
//            qDeleteAll(vlayoutOld->findChildren<QLineEdit *>());
//        }
...
//        qDeleteAll(controlsRestrictorWidget->findChildren<QVBoxLayout *>());
    ...
    ui->clockDomainSetGUI->setVisible(1);
}
...
void SetClockDomains::on_clockDomainSetGUI_clicked()
{
//    SetClockDomainGUI *mSetClockDomainGUI = new SetClockDomainGUI();
//    mSetClockDomainGUI->show();
//    this->hide();
    ...
    clockDomainData << QString::number(rows);
    clockDomainData << QString::number(cols);
...
//    qDebug() << clockDomainData;
    ...
    SetClockDomainGUI *mSetClockDomainGUI = new SetClockDomainGUI();
    connect(this, SIGNAL(submitClockDomainToGUISignal(QStringList)), mSetClockDomainGUI, SLOT(submitClockDomainDataToGUISlot(QStringList)));
    emit this->submitClockDomainToGUISignal(clockDomainData);
