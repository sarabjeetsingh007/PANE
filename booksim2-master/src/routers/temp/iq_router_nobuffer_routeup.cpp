...
std::deque<pair<int, pair<int, int> > > buffer_routeup[numRouters];
...
//		cout<<"Buffer:\n";
//		for(int C=0; C<numClients;C++)
//			cout<<buffer[this->GetID()][C].size()<<"\t";
//		cout<<"IQ:"<<_in_queue_flits.size();
//		cout<<"\tRU:"<<_route_vcs.size();
//		cout<<"\tVC:"<<_vc_alloc_vcs.size();
//		cout<<"\tSW:"<<_sw_alloc_vcs.size();
//		cout<<"\tCX:"<<_crossbar_flits.size();
//		cout<<"\tPC:"<<_proc_credits.size();
//		cout<<endl;	
								//*Sarab
	for(int P = 0; P<5; P++)
	{
		rv = poll(&fds[this->GetID()][0][P],1,0);
		if(rv == -1)
			perror("Poll Error");
		else if(rv == 0)
		{
//			printf("Timeout occurred!  \n");
		...
		{
			temp = 0;
			while ((char_rcv = fgetc(fp[this->GetID()][0][P])) != '*' && (char_rcv != EOF))		
			{
				if (char_rcv == '\n')
				{
//					cout<<"s: R,C,P:["<<this->GetID()<<"][0]["<<P<<"]->ID:"<<temp<<endl;
//					if(this->GetID()==14 && P==4)
//					cout<<GetSimTime()<<": InputQueuing["<<this->GetID()<<"]->ID:"<<temp<<", at Port:"<<P<<endl;
  ...
  if(!_route_vcs.empty() || !buffer[this->GetID()][1].empty() || !_route_vcs.empty())		//Buffer_routerup replaced by _route_vcs
  {
//		cout<<"RU called for router"<<this->GetID()<<endl;					//*Sarab	
	for(int P = 0; P<5; P++)
	{
//		if(P==2 && this->GetID()==0)
//		cout<<"RU called\n";	
...
//			cout<<"RouteUpdate["<<this->GetID()<<"]->ID:"<<temp<<", at Port:"<<P<<endl;
    ...
    activity = activity || !_route_vcs.empty() || !buffer[this->GetID()][1].empty() || !_route_vcs.empty();	//Buffer_routerup replaced by _route_vcs
  }
  if(!_vc_alloc_vcs.empty())
  {
								//*Sarab
...
//				cout<<"VCUpdate["<<this->GetID()<<"]->ID:"<<temp<<", at Port:"<<P<<endl;
  ...
  {
								//*Sarab	
//	cout<<GetSimTime()<<": SwitchUpdate["<<this->GetID()<<"] called!\n";
	for(int P = 0; P<5; P++)
	{
		rv = poll(&fds[this->GetID()][3][P],1,0);
		if(rv == -1)
			perror("Poll Error");
		else if(rv == 0)
		{
//			printf("Timeout occurred!  \n");
		...
		{
			temp = 0;
			while ((char_rcv = fgetc(fp[this->GetID()][3][P])) != '*' && (char_rcv != EOF))		
			{
				if (char_rcv == '\n')
				{
//					if(this->GetID()==10 && P==2)
//						cout<<GetSimTime()<<": SwitchUpdate["<<this->GetID()<<"]->ID:"<<temp<<", at Port:"<<P<<endl;
...
int flag[5]={0,0,0,0,0};
  ...
  while(iter != _in_queue_flits.end())					//TODO: Block reading to the  sockets and unblock after loop!
  {
...
//	cout<<"RU_Start\n";
	while(!_route_vcs.empty())
	{
		...
		buffer_routeup[this->GetID()].push_back(make_pair(time,make_pair(input,vc)));
//		cout<<"Route_Update.receive(push:"<<_buf[input]->FrontFlit(vc)->id<<")\n";
		_route_vcs.pop_front();
	}
//	cout<<"RU_Start1\n";
	 deque<pair<int, pair<int, int> > >::iterator iter = buffer_routeup[this->GetID()].begin();
...
//			cout<<"Route_Update.pop(pop:"<<f->id<<")\n";
			std::deque<pair<int, pair<int, int> > >::iterator j = iter;
...
void IQRouter::_SWAllocEvaluate( )
{
  ...
  for(deque<pair<int, pair<pair<int, int>, int> > >::iterator iter = _sw_alloc_vcs.begin(); iter != _sw_alloc_vcs.end(); ++iter)
  {
	...
	Buffer const * const cur_buf = _buf[input];
    Flit const * const f = cur_buf->FrontFlit(vc);
//	cout<<"\n-----4------SwitchAlloc["<<this->GetID()<<"]->"<<f->id<<endl;
...
//		if(f->tail==false)
	...
	bool flag=false;
	if(f && f->tail==true && (expanded_output >= 0))		//*Sarab		//TODO: Check validity
	{		
