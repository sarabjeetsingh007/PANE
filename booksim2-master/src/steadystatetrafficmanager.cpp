...
//      cout << "class " << c << " latency change    = " << latency_change << endl;	//ShutUP
...
//      cout << "class " << c << " throughput change = " << accepted_change << endl;		//ShutUP
...
//      cout << "Average latency for class " << lat_exc_class << " exceeded " << _latency_thres[lat_exc_class] << " cycles. Aborting simulation." << endl;	//ShutUP
      converged = 0; 
      _sim_state  = draining;
      _drain_time = _time;
      break;
    ...
    if ( _sim_state == warming_up ) {
      if ( ( _warmup_periods > 0 ) ? 
	   ...
	   ( ( !_measure_latency || ( lat_chg_exc_class < 0 ) ) &&
	     ( acc_chg_exc_class < 0 ) ) ) {
//	cout << "Warmed up ..." <<  "Time used is " << _time << " cycles" <<endl;	//ShutUP
	clear_last = true;
	_sim_state = running;
      }
    } else if(_sim_state == running) {
  ...
  if ( _sim_state == running ) {
    ...
    if ( _measure_latency ) {
//      cout << "Draining all recorded packets ..." << endl;	//ShutUP	
      int empty_steps = 0;
